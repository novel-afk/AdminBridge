...
const EditLeadModal = ({ isOpen, onClose, onSuccess, lead }) => {
  ...
  useEffect(() => {
    if (isOpen && lead) {
      // Initialize form data with lead values
      setFormData({
        name: lead.name || '',
        email: lead.email || '',
        phone: lead.phone || '',
        ...
        language_score: lead.language_score || '',
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter users to only include counsellors and managers
        const relevantUsers = usersResponse.data.filter(user => 
          ['Counsellor', 'BranchManager'].includes(user.role)
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:8000/api/leads/${lead.id}/`,
        formData,
        {
          headers: {
      ...
      <div className="bg-white rounded-xl shadow-xl w-[700px] max-h-[90vh] overflow-y-auto m-4">
        <div className="sticky top-0 bg-white border-b border-[#1e1b4b]/20 p-6 flex justify-between items-center">
          <h1 className="text-2xl font-bold text-gray-800">
            {currentStep === 1 ? 'Edit Lead - Personal Info' : 'Edit Lead - Educational Info'}
          </h1>
          <button
            onClick={onClose}
