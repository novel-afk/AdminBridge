...
const CounsellorDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      setError(null);
      try {
        const response = await axios.get(`${API_BASE_URL}/counsellor/stats`, {
          headers: { Authorization: `Bearer ${accessToken}` }
        });
        if (response.data) {
  ...
  const studentStatusPieChartData = {
    labels: Object.keys(stats.studentStatusCount || {}),
    datasets: [
      {
        label: 'Student Status',
        data: Object.values(stats.studentStatusCount || {}),
        backgroundColor: [
          'rgba(54, 162, 235, 0.6)',
          'rgba(255, 206, 86, 0.6)',
          'rgba(255, 99, 132, 0.6)',
              ...
              <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div className="bg-white overflow-hidden shadow rounded-lg">
                  <div className="px-4 py-5 sm:p-6">
                    ...
                    <div className="h-64">
                      <Pie data={studentStatusPieChartData} options={{ maintainAspectRatio: false }} />
                    </div>
                  </div>
                </div>
                <div className="bg-white overflow-hidden shadow rounded-lg">
